...
import com.bettercarrots.item.client.RilyniumWindArmorRenderer;
...
public class RilyniumWindArmorItem extends ArmorItem implements GeoItem {
    ...
    public RilyniumWindArmorItem(ArmorMaterial material, Type type, Settings settings){super(material, type, settings);}

    private static final Map<ArmorMaterial, StatusEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, StatusEffectInstance>())
                    .put(ModArmorMaterials.ELECTRIC_RILYNIUM, new StatusEffectInstance(StatusEffects.SPEED, 400, 2, false, false, true))
                    .build();
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private RilyniumWindArmorRenderer renderer;

            @Override
            public BipedEntityModel<LivingEntity> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, BipedEntityModel<LivingEntity> original){
                if (this.renderer == null)
                    this.renderer = new RilyniumWindArmorRenderer();
